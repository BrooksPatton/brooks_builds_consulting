...
- [~] Step 4: committed (1acc650, all 31 pipeline files) but push BLOCKED: the sandbox's injected GitHub token lacks the `workflow` scope needed to push `.github/workflows/*`. Brooks: run on your host `sbx secret set claude-brooks-builds-consulting github -t "$(gh auth token)"`, then any session can `git push`. (Remote already switched from SSH to HTTPS for proxy credential injection.)
